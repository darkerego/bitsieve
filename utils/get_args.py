...
def get_args():
    ...
    strategy_opts.add_argument('-str', '--strat', dest='strategy', type=str, choices=['sar', 'custom'], default='SAR',
    ...
    mqtt_opts = parser.add_argument_group('Mqtt Trade Signal Options')
    mqtt_opts.add_argument('-mq', '--mqtt', dest='enable_mqtt', default=False, action='store_true', help='Enable mqtt '
                                                                                                         'signal receiver')
    mqtt_opts.add_argument('-mu', '--mq_uri', dest='mq_uri', default='mq.blackmirror.app:1883', help='Mqtt server in host:port format.')
    mqtt_opts.add_argument('-me', '-exclude', dest='exclude_mqtt_markets', default=None, help='Markets to ignore signals for.')
    mqtt_opts.add_argument('-ms', '--min_score', dest='min_score', default=50, help='Min score to execute signals on.')


    receiver_opts =  parser.add_argument_group('Ws Receiver Trade Signal Options')

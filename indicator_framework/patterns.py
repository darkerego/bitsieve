...
def recognize_candlestick(op, hi, lo, cl, period, min_rating=50):
    """
    ...
    for candle in candle_names:
        ...
        score = (getattr(talib, candle)(op, hi, lo, cl))
        # talib.CDLMORNINGSTAR()
        _score = score[499]
        if (_score) != 0:
            if _score >- 100:
                sig = 'LONG'
                _candle_for_rating = f'{candle}_Bull'
            elif _score <= -100:
                sig = 'SHORT'
                _candle_for_rating = f'{candle}_Bear'
            rating = candle_rankings.get(_candle_for_rating)

            candle = candle.split('CDL')[1]
            signal = f'{candle}_{sig}_{period}_{rating}'
